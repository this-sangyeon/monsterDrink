...
const productFlavor = document.querySelector('.product-text > .text-wrap > p:nth-of-type(2)');
...
let productInfoLink =[
    {
        name: ['monster'+ 'energy'],
        flavor: ['Flavor Profile 강렬한 맛의 오리지널 에너지 드링크, 몬스터 에너지!'],
    ...
    {
        name: ['mango loco'],
        flavor: ['Flavor Profile 매혹적인 망고 맛의 몬스터 에너지 망고 로코'],
    ...
    {
        name: ['pipeline punch'],
        flavor: ['<span>Flavor Profile</span> 패션후르츠, 오렌지, 구아바 등의 열대과일의'],
    ...
    },
    {
        ...
        flavor: ['<span>Flavor Profile</span> 저칼로리로 가볍고 시원한 맛'],
    ...
    {
        name: ['ultra citra'],
        flavor: ['<span>Flavor Profile</span> 저칼로리에 시트러스의 상큼함을 느낄 수 있는 맛'],
    ...
    {
        name: ['ultra paradise'],
        flavor: ['<span>Flavor Profile</span> 저칼로리에 이색적인 키위-라임 향'],
...
for(let i = 0; i < productDrinkNavList.length; i++){
    productDrinkNavList[i].addEventListener('click',(e)=>{
        ...
        if(productIndex === 0){
            productName.innerText = productInfoLink[productIndex].name[i];
            productFlavor.innerText = productInfoLink[productIndex].flavor[i];
            productInfoText.innerText = productInfoLink[productIndex].text[i];
            productSubInfo.innerText = productInfoLink[productIndex].sub[i];
        }
        if(productIndex === 1){
            productName.innerText = productInfoLink[productIndex].name[0];
            productFlavor.innerText = productInfoLink[productIndex].flavor[0];
            productInfoText.innerText = productInfoLink[productIndex].text[0];
            productSubInfo.innerText = productInfoLink[productIndex].sub[0];
        }
        if(productIndex === 2){
            productName.innerText = productInfoLink[productIndex].name[0];
            productFlavor.innerText = productInfoLink[productIndex].flavor[0];
